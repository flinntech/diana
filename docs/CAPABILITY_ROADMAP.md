...
| Email access | 7 | 4 | 1.75 | Gmail API, security concerns |
| Reminders/alarms | 8 | 2 | 4.0 | node-cron + notifications |
| **System Control** |||||
| Run shell commands | 9 | 2 | 4.5 | Powerful but dangerous |
| File system operations | 7 | 2 | 3.5 | Read/write/move files |
| App launching | 5 | 2 | 2.5 | OS-specific |
| Clipboard access | 6 | 1 | 6.0 | Quick win |
| **Communication** |||||
| Voice input (STT) | 8 | 3 | 2.67 | Whisper local or API |
| Voice output (TTS) | 6 | 2 | 3.0 | Edge TTS, Piper, etc. |
...
1. **Agent interface** - Define contract for all agents
2. **Plugin architecture** (2.67) - MCP servers for tool exposure
3. **Orchestrator routing** - Route requests to appropriate agents

### Phase 1: Quick Wins (Score 5.0+)
4. **Clipboard access** (6.0) - Instant data sharing
5. **Wikipedia lookup** (5.0) - Simple knowledge queries
6. **Reclaim.ai integration** (5.0) - Tasks + calendar in one

### Phase 2: Core Tools (Score 4.0-4.5)
7. **Shell commands** (4.5) - With approval flow
8. **Web search + page reading** (4.0) - External knowledge
9. **Reminders/alarms** (4.0) - Time-based triggers

### Phase 3: System & Voice (Score 3.0-3.5)
10. **File system operations** (3.5) - Read/write/move files
11. **Voice output (TTS)** (3.0) - Spoken responses

### Phase 4: Intelligence (Score 2.67)
12. **Multi-step planning** (2.67) - Agent loops, ReAct
13. **Voice input (STT)** (2.67) - Hands-free queries
...
3. **When**: Design now, implement incrementally
